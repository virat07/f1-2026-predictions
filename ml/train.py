...
def train_race_winner():
    """Train classifier: (race, constructor) → is_winner (0/1)."""
    print("\n[1/4] Training race winner model (Pointwise) …")
    X, y, enc = build_race_winner_dataset()
    # y is already 0/1 integers
    X = X.fillna(0)
    sample_w = _year_weights(X["year"])
    X_train, X_test, y_train, y_test, w_train, _ = train_test_split(
        X, y, sample_w, test_size=0.2, random_state=42
    )
    # Give higher weight to winners to handle imbalance
    w_train = np.where(y_train == 1, w_train * 5, w_train)
    ...
    # Accuracy is less useful for imbalanced 0/1, but let's show it
    acc = (clf.predict(X_test) == y_test).mean()
    print(f"  ✓ Race winner binary accuracy: {acc:.3f}")
...
def train_podium():
    """Train three classifiers: (race, constructor) → is_p1/p2/p3."""
    print("\n[2/4] Training podium model (Pointwise) …")
    ...
    for i, y in enumerate(y_list):
        ...
        # Class weighting for podium spots (rarer than not being on podium)
        w_train = np.where(y_train == 1, w_train * 5, w_train)
        
        clf = GradientBoostingClassifier(n_estimators=120, max_depth=4, random_state=42)
        clf.fit(X_train, y_train, sample_weight=w_train)
        acc = (clf.predict(X_test) == y_test).mean()
        print(f"  ✓ Podium P{i+1} binary accuracy: {acc:.3f}")
        models.append(clf)
